...
import { Plus, Pencil, ChevronLeft, ChevronRight, TrendingUp, Target } from "lucide-react";
import {
  FORECAST_PRODAVACI, FORECAST_SOFTVERI, FORECAST_TIPOVI_LICENCE, POTENCIJAL_STATUSI, STATUS_BOJE,
  inputCls, btnPrimary, btnSecondary, btnGhostIcon,
  currentMonthStr, fmtMonth,
} from "../lib/crm";
  ...
  return (
    <Modal title={initial ? "Uredi stavku forecasta" : "Nova stavka forecasta"} onClose={onClose} wide>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4">
        ...
        <Field label="Kupac" required hint="Odaberi iz baze potencijala ili upiši novu firmu">
...
function TrendChart({ data }) {
  ...
  const chartData = months.map((m) => {
    ...
    return { mjesec: fmtMonth(m), ukupno, prodano };
  });
  ...
  return (
    <div>
      ...
      <div
        ...
        style={{ gridTemplateColumns: "minmax(140px,220px) minmax(140px,220px) minmax(120px,180px) minmax(120px,180px)" }}
      >
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5">
        <h3 className="text-sm font-semibold text-slate-800 mb-4 flex items-center gap-1.5">
          <TrendingUp size={15} className="text-slate-400" /> Trend — potencijalne vs prodane licence (zadnjih 12 mjeseci)
        </h3>
